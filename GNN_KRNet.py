...
class My_UW_Loss(nn.Module):
    ...
    def __init__(self):
        ...
        self.log_vars = [self.a, self.b, self.c]
...
class Gnn_KRNet(torch.nn.Module):
    def __init__(self, input_dim=10, batch_size=256, encoder_dim=5):
        ...
        class_num = 7
        self.represent_layer = nn.Linear(self.vector_num * encoder_dim, class_num)
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        for batch in loaded_data:
            ...
            loss2 = criteria(output, targets)
            loss = loss_function([loss1, loss2, loss0])
        ...
        if (epoch + 1) % 10 == 0:
            ...
            # 保存模型参数到文件
            torch.save(model.state_dict(), 'parameter_loss_all.pth')
